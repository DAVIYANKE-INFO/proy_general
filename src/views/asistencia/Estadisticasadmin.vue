...
<v-layout row wrap>
    ...
    <v-flex sm12>
     ...
     <h1 style="color:blue;font-family:roboto;font-size:18px;">Rango de fechas por defecto : </h1>
        <br>
        <h2 style="color:black;text-align:center;">{{ fechainicio }} {{ " hasta " }} {{ fechafin }}</h2>
        

         <h3 v-if="usuarios.length==0" style="color:red;text-align:right;"></h3>
        <h3 v-else style="color:red;text-align:right;"> {{ usuarios }}  </h3>

        <h4 v-if="usuarios.length==0" style="color:green;text-align:right;"></h4>
        <h4 v-else style="color:green;text-align:right;"> {{ usuario_material[usuario_material.findIndex(mascota => mascota.id === usuarios)].fechaingreso }}  </h4>
                ...
                <template v-slot:item="data">
                  ...
                  <!--  <v-list-item-avatar>
                    ...
                    <v-list-item-content>
                      ...
                      <!--<v-list-item-subtitle v-html="data.item.fechaingreso"></v-list-item-subtitle>-->
                    </v-list-item-content>
                  </template>
                </template>
            </v-autocomplete>
          <v-btn small text color="red" @click="reiniciar()">LIMPIAR</v-btn>
        </v-card-text>
     </v-card>
     </v-flex>
    <v-flex sm6>
...
:elevation="hover ? 24 : 6"
        ...
        tile
    >
    <h3 style="color:blue;">GENERAR REPORTE DE USUARIO</h3>
    <v-card
        class="ma-1 pa-3"
        outlined
        tile
    ...
    <v-col class="text-center" cols="12" lg="4">
        ...
        >
          <template v-slot:activator="{ on }">
            <v-text-field
              v-model="datesito"
              label="Selección fecha inicio"
              prepend-icon="event"
              readonly
              v-on="on"
            ></v-text-field>
          </template>
          <v-date-picker locale="es" v-model="datesito" no-title scrollable @input="$refs.menufecha.save(datesito)">
          </v-date-picker>
        </v-menu>
        </v-col>
        <v-col cols="12" lg="4">
        ...
        >
          ...
          <v-date-picker locale="es" v-model="date1" no-title scrollable @input="$refs.menufecha1.save(date1)">
          </v-date-picker>
        </v-menu>
        </v-col>
...
export default 
{
  ...
  data(){
    ...
    datesito: new Date().toISOString().substr(0, 10),
    menufecha: false,
  ...
  computed: {
    ...
    loader () {
      ...
      methods:{
        mesusu()
      {
        console.log(" xx> ",this.usuarios);
        console.log(" xx> ",this.datesito);
        console.log(" xx> ",this.date1);
          let compa = this;
          axios.post(compa.store+'/reporte_usuario',{
            carnetid:compa.usuarios,
            fechaini:compa.datesito +"",
            fechafin:compa.date1+"",
          }).then(function (response) 
          { 
        ...
        {
          this.usuarios=[];
          //this.usuario_material=[];
        },

          consulta321()
          {
                    let compa = this;
                    axios.post(compa.store+'/meses_reportes',
                    {
                        carnetsito:compa.usuarios+"",
                    }).then(function (response)
                    {
                        console.log("rreportes meses ", response.data); 
                        for(var u=0;u<response.data.length;u++)
                        {
                            compa.mydatasets[u].data=response.data[u].datitos;
                        }  
                        console.log("este es david ", compa.mydatasets)
                    }).
                    catch(function (error)
                    {
                        console.log("error", error)
                    ...
                    console.log("ciii ", this.usuarios)//resultado = response.data;
                        axios.post(compa.store+'/consultaestad',{
                          codigousu:compa.usuarios+"",
                          yyy: "si",
                    }).then(function (response)
                    {
                       console.log("cxcxc ", response.data);//resultado = response.data;
